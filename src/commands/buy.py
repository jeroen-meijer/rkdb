...
def buy_tracks():
  ...
  missing_tracks_without_ignored = {k: v for k, v in missing_tracks_db.items() if not v.get("ignored", False)}
  ...
  try:
    for i in range(total_missing_tracks_to_process):
      ...
      counter = f"{str(i+1).rjust(len(str(total_missing_tracks_to_process)))}/{total_missing_tracks_to_process}"
      prompt = f"[{counter}] {_bold(full_track_str)} · Did you buy this track? ({_bold('Y')}/N/I/R/Q)"
      ...
      with yaspin(text=prompt, color="blue") as sp:
        ...
        possible_actions = [_BuyAction.YES, _BuyAction.NO, _BuyAction.IGNORE, _BuyAction.QUIT]
        while action not in possible_actions:
          ...
          action = next((a for a in possible_actions if a.value == res), None) if res != "\n" else _BuyAction.YES
          if action == None:
            # Clear line
  ...
  total_missing_tracks_to_process_afterwards = total_missing_tracks_to_process - total_tracks_processed

  print("🎉 Done!")
  print(f"  - ✅ {_bold(total_tracks_processed)} out of {total_missing_tracks_to_process} tracks processed.")
  print(f"  - 🛒 {_bold(actions_performed.get(_BuyAction.YES, 0))} tracks purchased.")
  print(f"  - 🚫 {_bold(actions_performed.get(_BuyAction.IGNORE, 0))} tracks ignored.")
  print(f"  - 📦 {_bold(total_missing_tracks_to_process_afterwards)} tracks left to process.")
