...
def sync_spotify_playlists_to_rekordbox():
  ...
  rb_all_tracks: List[r.db6.tables.DjmdContent] = list(filter(lambda track: track.Title != None and track.Artist != None, rb.get_content()))
  print('Fetching Rekordbox keys...')
  camelot_key_starts = tuple(str(n+1) for n in range(12))
  rb_camelot_keys: dict[str, r.db6.tables.DjmdKey] = {k.ScaleName.upper(): k for k in list(rb.get_key()) if k.ScaleName.startswith(camelot_key_starts)}

  if (len(rb_camelot_keys) != 24):
    raise ValueError(f"Expected 24 keys but found {len(rb_camelot_keys)}: {list(map(lambda k: k.ScaleName, rb_camelot_keys))}")
  ...
  sp_flow_playlists = list(filter(lambda playlist: playlist['name'].startswith('FLOW'), all_playlists))
  sp_set_playlists = list(filter(lambda playlist: playlist['name'].startswith('SET'), all_playlists))
  ...
  def sync_playlist(sp_playlist):
    ...
    rb_playlist_key: r.db6.tables.DjmdKey | None = rb_camelot_keys.get(sp_playlist_camelot_key, None)
    ...
    sp_playlist_tracks = exhaust_fetch(
      ...
      map_elements = lambda res: list(map(lambda item: item['track'], res['items']))
    )

    log(f"Creating playlist")
    rb_playlist_with_same_name = first_or_none(filter(lambda playlist: playlist.Name == sp_playlist_name, rb_playlists))
    ...
    if playlist_folder_name != None:
      playlist_folder = first_or_none(filter(lambda playlist: playlist.Name == playlist_folder_name and playlist.is_folder, list(rb.get_playlist())))
    ...
    for sp_track in sp_playlist_tracks:
      sp_track_artist_str = ', '.join(list(map(lambda artist: artist['name'], sp_track['artists'])))
      ...
      def attempt_add_track_to_missing_db():
        ...
        itunes_url: str | None = missing_tracks_db.get(sp_track['id'], {}).get('itunes_url', None)
        ...
        if itunes_url != None:
          ...
          try:
            ...
            itunes_search_res: List[iGet.iGet.song] = list(filter(lambda content: content.kind == 'song', iGet.get(term=sp_track_full_str, country='NL')))
            itunes_song = find_best_match(sp_track_name_str, lambda song: song.trackName, itunes_search_res)
            itunes_url = generate_itunes_store_url(itunes_song) if itunes_song != None else None
            if itunes_url != None:
      ...
      rb_track = first_or_none(filter(lambda track: track.ID == rb_track_id, rb_all_tracks)) if rb_track_id != None else None
      if rb_track != None:
        log(f"└ ✅ Found ID match:      {rb_track.ID}")
      else:
        search_results = find_track({'artist': sp_track_artist_str, 'title': sp_track_name_str}, rb_all_tracks)
        ...
        if rb_track != None:
          ...
          log(f"└ ✅ Found closest match: \"{rb_track.ArtistName} - {rb_track.Title}\" ({match_percentage}%)")
    ...
    log(f"Finished syncing playlist in {humanfriendly.format_timespan(end_datetime - start_datetime)}")
  ...
  try:
    ...
    end_datetime = datetime.datetime.now()
    print(f"Synced all playlists in {humanfriendly.format_timespan(end_datetime - start_datetime)}")
  except Exception as e:
